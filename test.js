...
import { getUploadPublicSignature } from '@mdt/apis/dist/upload'
import queryUrl from '../../../js/utils/queryUrl';
import deviceType from '../../../js/utils/deviceType';
...
export const isCordovaDevice = function() {
    return deviceType() !== deviceType.types.BROWSER;
}
 ...
 * @param {*} picUrl 
 */
export const getDataUrlFromPath = function(picUrl) {
    console.log('picUrl:', picUrl)
    picUrl = 'file://' + picUrl;
    return new Promise((resolve) => {
        window.resolveLocalFileSystemURL(picUrl, function(fileEntry) {
 ...
 * @param {*} fileOrPath 
 */
export const getFileDataUrl = function(fileOrPath) {
    return new Promise((resolve) => {
        if (isCordovaDevice()) {
            getDataUrlFromPath(fileOrPath.path).then(({dataUrl, err}) => {
                if (dataUrl) {
                    resolve(dataUrl);
                }else {
                    resolve(err);
...
function upload(file, host, params, progressCallback, succ, fail) {
    ...
    request.onload = function(evt) {
        if (request.status == 200) {
            ...
            succ && succ({oss_path: `${host}/${params.key}`, path:params.key});
        }else{
...
export const getUploadAuth = function(file, md5, succ, fail, uploadType) {
    ...
    getUploadPublicSignature({upload_type: uploadType}, params).then(resp => {
            if (resp && !resp.Status) {
                succ && succ(resp);
            }else {
                fail && fail(resp);
...
export const uploadFile = function(fileOrPath, uploadType, progressCallback, succ, fail) {
    if(uploadType === "snapshot" && deviceType() === deviceType.types.IOS){
        getBlobMd5(fileOrPath, (file, md5, err) => {
            if (file && md5 !== null) {
                if(!size(file.name)){
                    file.name = "map_" + (new Date()).getTime();
                }
                getUploadAuth(file, md5, (resp) => {
                    ...
                    let params = {
                        'name': name,
                        'key': resp['path'],
                        'policy': resp['policy'], //oss
                        'OSSAccessKeyId': resp['accessid'], //oss
                        'success_action_status': '200', //oss 让服务端返回200,不然，默认会返回204
                        'signature': resp['signature'], //oss
                        'Policy': resp['policy'], //minio
                        'X-Amz-Algorithm': resp['x-amz-algorithm'], //minio
                        'X-Amz-Credential': resp['x-amz-credential'], //minio
                        'X-Amz-Date': resp['x-amz-date'], //minio    
                        'X-Amz-Signature': resp['x-amz-signature'], //minio
                    };
